...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            responseHandler: function(res) {
                ...
                if (res.bizCode == 0) {
                    if ($.common.isNotEmpty($.table._option.sidePagination) && $.table._option.sidePagination === 'client') {
                        ...
                        return { rows: res.data.rows, total: res.data.totalNum, colSum: res.data.colSum };
                    }
                } else if (res.bizCode == web_status.LOGIN_TIMEOUT) {
					window.location.href = ctx + "/login"
				} else {
                    $.modal.alertWarning(res.bizMsg);
                    return { rows: [], total: 0 };
                }
            },
			// 初始化事件
                ...
                $.modal.confirm("确定导出所有" + $.table._option.modalName + "吗？", function() {
                    ...
                    $.post($.table._option.exportUrl, $("#" + currentId).serializeArray(), function(result) {
                        if (result.bizCode == web_status.SUCCESS) {
                            window.location.href = ctx + "common/download?fileName=" + encodeURI(result.bizMsg) + "&delete=" + true;
                        } else if (result.bizCode == web_status.WARNING) {
                            $.modal.alertWarning(result.bizMsg)
                        } else {
                            $.modal.alertError(result.bizMsg);
                        }
                        $.modal.closeLoading();
                    });
                });
                    ...
                    btn1: function(index, layero){
                        ...
                        $.ajax({
                            ...
                            success: function (result) {
                                if (result.bizCode == web_status.SUCCESS) {
                                    $.modal.closeAll();
                                    $.modal.alertSuccess(result.bizMsg);
                                    $.table.refresh();
                                } else if (result.bizCode == web_status.WARNING) {
                                    layer.close(index);
                                    $.modal.enable();
            ...
            selectDictLabel: function(datas, value) {
                ...
                $.each(datas, function(index, dict) {
                    if (dict.value == value) {
                    	actions.push("<span class='badge badge-" + dict.listClass + "'>" + dict.labelName + "</span>");
                        return false;
                    }
                });
                return actions.join('');
    	...
    	modal: {
            ...
            // 错误提示
            alertError: function(result) {
    			if (!result.bizCode) {
					$.modal.alert(result, modal_status.FAIL);
    				return
				}
    			if (result.bizCode == web_status.LOGIN_TIMEOUT) {
					window.location.href = ctx + "/login"
				} else {
					$.modal.alert(result.bizMsg, modal_status.FAIL);
				}
            },
            // 成功提示
        ...
        operate: {
        	// 提交数据
        	submit: function(url, data, ajaxConfig) {
        		if (!ajaxConfig) {
					ajaxConfig = {}
				}
        		if (ajaxConfig.contentType && ajaxConfig.contentType.indexOf("application/json") != -1) {
            	...
            	var config = {
        	        ...
        	        data: data,
					contentType: ajaxConfig.contentType || "application/x-www-form-urlencoded;charset=utf-8",
        	        beforeSend: function () {
            ...
            ajaxSuccess: function (result) {
                if (result.bizCode == web_status.SUCCESS && $.table._option.type == table_type.bootstrapTable) {
                    $.modal.msgSuccess(result.bizMsg);
                    $.table.refresh();
                } else if (result.bizCode == web_status.SUCCESS && $.table._option.type == table_type.bootstrapTreeTable) {
                    $.modal.msgSuccess(result.bizMsg);
                    $.treeTable.refresh();
                } else if (result.bizCode == web_status.WARNING) {
                    $.modal.alertWarning(result.bizMsg)
            ...
            successCallback: function(result) {
                if (result.bizCode == web_status.SUCCESS) {
                    ...
                    if (parent.$.table._option.type == table_type.bootstrapTable) {
                        $.modal.close();
                        window.parent.$.modal.msgSuccess(result.bizMsg);
			...
			successTabCallback: function(result) {
				if (result.bizCode == web_status.SUCCESS) {
					...
					$.modal.close();
					$contentWindow.$.modal.msgSuccess(result.bizMsg);
        ...
        tree: {
        	...
        	init: function(options) {
        	    ...
        	    var setting = {
        	        ...
        	        data: {
        	    		key: {
        	    			id: options.dataId || "id",
        	        		// title: options.dataTitle || "menuName",
							name: options.dataName || "name",
