...
import com.linzx.framework.es.bean.*;
...
public class ElasticsearchTemplateImpl<T, M> implements ElasticsearchTemplate<T, M> {
    ...
    public boolean save(T t) throws Exception {
        MetaData metaData = IndexTools.createMetaData(t.getClass());
        String indexName = metaData.getIndexName();
        String indexType = metaData.getIndexType();
        String id = IndexTools.getESId(t);
        ...
        String source = JSONObject.toJSONString(t);
    ...
    public BulkResponse save(List<T> list) throws Exception {
        ...
        for (int i = 0; i < list.size(); i++) {
            T tt = list.get(i);
            String id = IndexTools.getESId(tt);
            String source = JSONObject.toJSONString(t);
    ...
    public boolean update(T t) throws Exception {
        ...
        updateRequest.doc(IndexTools.getFieldValIfNotNull(t));
...
//                        }
    ...
    public List<T> scroll(QueryBuilder queryBuilder, Class<T> clazz) throws Exception {
        return scroll(queryBuilder, clazz, DEFAULT_SCROLL_TIME);
    }
    ...
    public T getById(M id, Class<T> clazz) throws Exception {
        ...
        GetRequest getRequest = new GetRequest(indexName, indexType, idStr);
    ...
    public List<T> mgetById(M[] ids, Class<T> clazz) throws Exception {
        ...
        for (int i = 0; i < ids.length; i++) {
            String idStr = Convert.toStr(ids[i]);
            request.add(new MultiGetRequest.Item(indexName, indexType, idStr));
        }
...
//        aggregation.order(BucketOrder.key(false));
    ...
    public Map percentilesAggs(String metricName, QueryBuilder queryBuilder, Class<T> clazz) throws Exception {
        return percentilesAggs(metricName, queryBuilder, clazz, DEFAULT_PERCSEGMENT);
    }
